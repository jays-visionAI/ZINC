/**
 * scripts/sync-channel-icons.js
 * Synchronizes hardcoded SVG icons from utils-channel-selector.js 
 * to the channelProfiles collection in Firestore.
 */

(async function syncIcons() {
    console.log("🚀 Starting Channel Icon Synchronization...");

    if (typeof db === 'undefined' || typeof firebase === 'undefined') {
        console.error("❌ Firebase/Firestore not initialized. Please ensure firebase-config.js is loaded.");
        return;
    }
    ...
    for (const key in CHANNEL_LABELS) {
        const docRef = db.collection('channelProfiles').doc(key);
        ...
        batch.set(docRef, {
            ...
            status: 'active',
            updatedAt: timestamp,
            supportsApiConnection: true // Default assumed for these mappings
        }, { merge: true });

        console.log(`  ✅ Queued Sync: ${name} (${key})`);
    }

    try {
        await batch.commit();
        console.log("\n✨ Successfully synchronized all channel icons and metadata!");
        alert("✅ Channel icons and metadata synchronized successfully!");
    } catch (error) {
